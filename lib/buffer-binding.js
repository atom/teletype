...
class BufferBinding {
  constructor ({buffer, isHost, didDispose}) {
    ...
    this.subscriptions.add(buffer.onDidChangePath(this.setPathDidChange.bind(this)))
  }
  ...
  setPathDidChange(newPath){
    if(this.isHost){
      this.bufferProxy.requestPathChanged(this.getBufferProxyURI())
    }
  }
  ...
  addFile (in_bufferProxy) {
    const bufferFile = new BufferFile({bufferProxy: in_bufferProxy, buffer: this.buffer})
    this.buffer.setFile(bufferFile)
    return bufferFile
  }
