...
class BufferBinding {
  ...
  setSharedBuffer (sharedBuffer) {
    ...
    this.subscriptions.add(this.textBuffer.onDidChangeText(this.relayLocalChanges.bind(this)))
  }

  relayLocalChanges (changes) {
    const operations = []
    for (let i = 0; i < changes.length; i++) {
      const {oldRange, newRange, newText} = this.sharedBuffer.changes[i]
      const position = this.textBuffer.characterIndexForPosition(oldRange.start)
      const oldExtent = this.textBuffer.characterIndexForPosition(oldRange.end) - position
