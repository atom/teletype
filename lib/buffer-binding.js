...
class BufferBinding {
  ...
  setBufferProxy (bufferProxy) {
    ...
    if (this.isHost !== undefined && !this.isHost) {
      this.addFile(bufferProxy.uri)
    }
  ...
  didChangeURI (uri) {
    if (this.bufferFile) this.bufferFile.setURI(uri)
  }
  ...
  addFile (inURI) {
    this.bufferFile = new BufferFile({uri: inURI})
    this.buffer.setFile(this.bufferFile)
  }
