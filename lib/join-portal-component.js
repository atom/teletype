...
class JoinPortalComponent {
  ...
  writeAfterUpdate () {
    ...
    if (this.refs.editor) this.refs.editor.component.didUpdateStyles()
  }
  ...
  render () {
    ...
    if (joining) {
      ...
      return $.div({className: 'JoinPortalComponent--prompt'},
        $(TextEditor, {ref: 'editor', mini: true, placeholderText: 'Enter a host portal ID here...'}),
        $.button({type: 'button', className: 'btn', onClick: this.joinPortal}, 'Join')
      )
    } else {
      return $.div({className: 'JoinPortalComponent--no-prompt'},
        $.label({onClick: this.showPrompt}, 'Join Portal')
      )
    }
  }
  ...
  async joinPortal () {
    ...
    const portalId = this.refs.editor.getText()
