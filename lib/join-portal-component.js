...
class JoinPortalComponent {
  ...
  writeAfterUpdate () {
    ...
    if (this.refs.portalIdEditor) this.refs.portalIdEditor.component.didUpdateStyles()
  }

  render () {
    const {joining, promptVisible} = this.props
    if (joining) {
      return $.div({ref: 'joiningSpinner', className: 'JoinPortalComponent--no-prompt'},
        $.span({className: 'loading loading-spinner-tiny inline-block'})
      )
    } else if (promptVisible) {
      return $.div({className: 'JoinPortalComponent--prompt'},
        $(TextEditor, {ref: 'portalIdEditor', mini: true, placeholderText: 'Enter a host portal ID here...'}),
        $.button({type: 'button', className: 'btn', onClick: this.joinPortal}, 'Join')
      )
    } else {
      return $.div({className: 'JoinPortalComponent--no-prompt'},
        $.label({ref: 'joinPortalLabel', onClick: this.showPrompt}, 'Join Portal')
      )
    }
  }

  async showPrompt () {
    await this.update({promptVisible: !this.props.promptVisible})
  }
  ...
  async joinPortal () {
    ...
    if (await portalBindingManager.getGuestPortalBinding(portalId)) {
