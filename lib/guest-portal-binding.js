...
class GuestPortalBinding {
  constructor ({portal, workspace, notificationManager, didLeave, hostDidDisconnect}) {
    ...
    this.screenSharePaneItem = new ScreenSharePaneItem()
  ...
  async setActiveTextEditor (clientEditor) {
    if (clientEditor == null) {
      await this.replaceActivePaneItem(this.screenSharePaneItem)
    } else {
  ...
  addScreenShareTrack (track) {
    this.screenSharePaneItem.setScreenShareTrack(track)
  }
