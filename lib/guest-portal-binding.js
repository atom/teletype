...
class GuestPortalBinding {
  ...
  siteDidJoin (siteId) {
    const {username: hostUsername} = this.portal.getSiteIdentity(1)
    const {username: siteUsername} = this.portal.getSiteIdentity(siteId)
    this.notificationManager.addInfo(`@${siteUsername} has joined @${hostUsername}'s portal'`)
  }

  siteDidLeave (siteId) {
    const {username: hostUsername} = this.portal.getSiteIdentity(1)
    const {username: siteUsername} = this.portal.getSiteIdentity(siteId)
    this.notificationManager.addInfo(`@${siteUsername} has left @${hostUsername}'s portal'`)
  }
