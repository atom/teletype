...
class GuestPortalBinding {
  ...
  getRemoteBuffers () {
    // TODO: Remove duplicate URIs. We may have some if there are multiple editors for the same buffer (e.g. after a split).
    ...
    return Array.from(this.editorProxiesById.values()).map((editorProxy) => {
      const {bufferProxy} = editorProxy
      return {
        uri: bufferProxy.uri,
        label: `${uriPrefix}: ${path.basename(normalizeURI(bufferProxy.uri))}`
      }
    })
  }

  async getRemoteEditor ({bufferProxyId, editorProxyId}) {
    await this.portal.findBufferProxy(bufferProxyId)
    const editorProxy = await this.portal.findEditorProxy(editorProxyId)
