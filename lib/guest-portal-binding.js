...
class GuestPortalBinding {
  ...
  addEditorProxy (editorProxy) {
    // TODO: Delete calls to this method, and then delete this method too.
    this.editorProxiesMetadataById.set(editorProxy.id, editorProxy)
  }

  removeEditorProxy (editorProxy) {
    this.editorProxiesMetadataById.delete(editorProxy.id)
  ...
  getRemoteEditors () {
    // TODO: Decide what to do with editors backed by the same buffer. We may have some if an editor has been split.
    // TODO: Remove duplication between this method and EditorBinding.monkeyPatchEditorMethods: See https://github.com/atom/teletype/pull/272
    const hostIdentity = this.portal.getSiteIdentity(1)
    const uriPrefix = hostIdentity ? `@${hostIdentity.login}` : 'remote'
    return Array.from(this.editorProxiesMetadataById.values()).map((editorProxyMetadata) => {
      const {id, bufferProxyURI} = editorProxyMetadata
      return {
        hostGitHubUsername: hostIdentity ? hostIdentity.login : null,
        uri: 'teletype://' + this.portal.id + '/editor/' + id,
        path: bufferProxyURI,
        label: `${uriPrefix}: ${path.basename(normalizeURI(bufferProxyURI))}`
      }
    })
  }
