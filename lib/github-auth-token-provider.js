...
class GithubAuthTokenProvider {
  constructor ({commandRegistry, workspace, openURL, credentialCache}) {
  ...
  getToken () {
    return new Promise(async (resolve, reject) => {
      const password = await this.credentialCache.get('oauth-token')
      if (password) {
        resolve(password)
      } else {
  ...
  async confirm () {
    ...
    await this.credentialCache.set('oauth-token', password)
