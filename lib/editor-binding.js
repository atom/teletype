...
class EditorBinding {
  ...
  dispose () {
    ...
    this.upwardSitePositionsComponent.destroy()
    this.downwardSitePositionsComponent.destroy()
    this.leftwardSitePositionsComponent.destroy()
    this.rightwardSitePositionsComponent.destroy()
  ...
  setEditorProxy (editorProxy) {
    ...
    this.upwardSitePositionsComponent = new SitePositionsComponent({portal: this.portal, position: 'top', onSelectSiteId: follow})
    this.downwardSitePositionsComponent = new SitePositionsComponent({portal: this.portal, position: 'bottom', onSelectSiteId: follow})
    this.leftwardSitePositionsComponent = new SitePositionsComponent({portal: this.portal, position: 'left', onSelectSiteId: follow})
    this.rightwardSitePositionsComponent = new SitePositionsComponent({portal: this.portal, position: 'right', onSelectSiteId: follow})

    this.editor.element.appendChild(this.upwardSitePositionsComponent.element)
    this.editor.element.appendChild(this.downwardSitePositionsComponent.element)
    this.editor.element.appendChild(this.leftwardSitePositionsComponent.element)
    this.editor.element.appendChild(this.rightwardSitePositionsComponent.element)
  }
  ...
  updateActivePositions (positionsBySiteId) {
    ...
    const upwardSiteIds = []
    const downwardSiteIds = []
    const leftwardSiteIds = []
    const rightwardSiteIds = []
    ...
    for (let siteId in positionsBySiteId) {
      ...
      if (screenPosition.row < element.getFirstVisibleScreenRow()) {
        upwardSiteIds.push(siteId)
      } else if (screenPosition.row > element.getLastVisibleScreenRow()) {
        downwardSiteIds.push(siteId)
      } else if (pixelPosition.left < element.getScrollLeft()) {
        leftwardSiteIds.push(siteId)
      } else if (pixelPosition.left > element.getScrollRight()) {
        rightwardSiteIds.push(siteId)
      }
    }

    this.upwardSitePositionsComponent.update({siteIds: upwardSiteIds})
    this.downwardSitePositionsComponent.update({siteIds: downwardSiteIds})
    this.leftwardSitePositionsComponent.update({siteIds: leftwardSiteIds})
    this.rightwardSitePositionsComponent.update({siteIds: rightwardSiteIds})
