...
const SitePositionsComponent = require('./site-position-indicators-component')
...
class EditorBinding {
  constructor ({editor, portal, isHost, didDispose}) {
    ...
    this.upwardSitePositionsComponent = new SitePositionsComponent({position: 'top'})
    this.downwardSitePositionsComponent = new SitePositionsComponent({position: 'bottom'})
    this.leftwardSitePositionsComponent = new SitePositionsComponent({position: 'left'})
    this.rightwardSitePositionsComponent = new SitePositionsComponent({position: 'right'})
  }
  ...
  updateActivePositions (positionsBySiteId) {
    ...
    for (const siteId in positionsBySiteId) {
      ...
      const siteIdentity = this.portal.getSiteIdentity(parseInt(siteId))

      if (screenPosition.row < element.getFirstVisibleScreenRow()) {
        upwardSites.push(siteIdentity)
      } else if (screenPosition.row > element.getLastVisibleScreenRow()) {
        downwardSites.push(siteIdentity)
      } else if (pixelPosition.left < element.getScrollLeft()) {
        leftwardSites.push(siteIdentity)
      } else if (pixelPosition.left > element.getScrollRight()) {
        rightwardSites.push(siteIdentity)
      }
    }

    this.upwardSitePositionsComponent.update({sites: upwardSites})
    this.downwardSitePositionsComponent.update({sites: downwardSites})
    this.leftwardSitePositionsComponent.update({sites: leftwardSites})
    this.rightwardSitePositionsComponent.update({sites: rightwardSites})
