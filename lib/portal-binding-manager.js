...
class PortalBindingManager {
  ...
  async getRemoteBuffers () {
    ...
    for (const bindingPromise of this.promisesByGuestPortalId.values()) {
      ...
      remoteBuffers.push(...portalBinding.getRemoteBuffers())
    }
  ...
  async getRemoteEditorForURI (uri) {
    const portalId = uri.host
    const uriComponents = uri.pathname.split('/')
    const bufferProxyId = Number(uriComponents[2])
    const editorProxyId = Number(uriComponents[4])
    ...
    if (guestPortalBindingPromise) {
      ...
      return guestPortalBinding.getRemoteEditor({bufferProxyId, editorProxyId})
    } else {
