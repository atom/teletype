...
let Client
allowUnsafeNewFunction(() => { RealTimeClient = require('@atom/real-time-client') })
...
class RealTimePackage {
  constructor (options) {
    const {
      ...
      restGateway, pubSubGateway, pusherKey, baseURL, testEpoch, getScreenShareStream
    } = options
    ...
    if (getScreenShareStream) this.getScreenShareStream = getScreenShareStream
  ...
  observeActiveTextEditor () {
    this.hostPortalDisposables.add(
  ...
  async shareScreen () {
    ...
    this.hostPortal.addScreenShareTrack(stream.getTracks()[0], stream)
  }
