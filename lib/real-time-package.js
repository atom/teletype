...
class RealTimePackage {
  constructor ({workspace, commands, clipboard, restGateway, pubSubGateway, pusherKey, baseURL}) {
    ...
    // TODO Can/should we combine these into a single object? Or give them better names?
    this.editorsBySharedEditor = new WeakMap() // Used by guest
    this.sharedEditorsByEditor = new WeakMap() // Used by host
  }
  ...
  async joinPortal () {
    ...
    portal.setDelegate({
      setActiveSharedEditor: async (sharedEditor) => {
        let editor = this.editorsBySharedEditor.get(sharedEditor)
        if (editor == null) {
          ...
          this.editorsBySharedEditor.set(sharedEditor, editor)
