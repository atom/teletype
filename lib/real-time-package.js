...
class RealTimePackage {
  ...
  async joinPortal (portalId) {
    const client = await this.getClient()
    if (client) {
      let portalBinding = this.guestPortalBindings.find((b) => b.portalId === portalId)
      if (portalBinding) {
        portalBinding.activate()
        return portalBinding.portal
      } else {
        portalBinding = new GuestPortalBinding({
          client,
          portalId,
          workspace: this.workspace,
          notificationManager: this.notificationManager,
  ...
  async getClient () {
    try {
      ...
      const message = 'Failed to initialize the real-time package'
      const description = `Establishing a real-time connection failed with error: <code>${error.message}</code>`
