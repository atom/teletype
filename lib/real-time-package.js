...
const KeytarPasswordManager = require('./keytar-password-manager')
...
class RealTimePackage {
  constructor (options) {
    const {
      ...
      authenticationProvider, restGateway, pubSubGateway, pusherKey, baseURL
    } = options
    ...
    this.authenticationProvider = authenticationProvider || new GithubAuthenticationProvider({
      workspace,
      commandRegistry,
      passwordManager: new KeytarPasswordManager(),
  ...
  async getClient () {
    try {
      const oauthToken = await this.authenticationProvider.authenticate()
      await this.client.initialize({oauthToken})
      return this.client
    } catch (error) {
