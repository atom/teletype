...
class RealTimePackage {
  constructor (options) {
    const {
      workspace, commandRegistry, clipboard, restGateway, pubSubGateway,
      pusherKey, baseURL, heartbeatIntervalInMilliseconds,
  ...
  async joinPortal (portalId) {
    ...
    const portalBinding = new GuestPortalBinding({
      workspace: this.workspace,
      notificationManager: this.workspace.notificationManager
    })
