...
class RealTimePackage {
  constructor (options) {
    ...
    this.editorProxiesByEditor = null
  ...
  async sharePortal () {
    ...
    const activeTextEditorDisposable = this.workspace.observeActiveTextEditor(async (editor) => {
      if (editor == null) {
        await this.hostPortal.setActiveEditorProxy(null)
        return
      }

      let editorProxy = this.editorProxiesByEditor.get(editor)
      if (editorProxy == null) {
        const buffer = editor.getBuffer()
        const bufferBinding = new BufferBinding(buffer)
        const bufferProxy = await this.hostPortal.createBufferProxy({
        ...
        editorProxy = await this.hostPortal.createEditorProxy({
          bufferProxy,
      ...
      await this.hostPortal.setActiveEditorProxy(editorProxy)
    })
