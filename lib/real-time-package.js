...
class RealTimePackage {
  ...
  async sharePortal () {
    ...
    this.workspace.observeActiveTextEditor(async (editor) => {
      if (editor == null) {
        return await portal.setActiveSharedEditor(null)
      }
  ...
  async joinPortal (portalId) {
    ...
    portal.setDelegate({
      setActiveSharedEditor: async (sharedEditor) => {
        let editor
        if (sharedEditor == null) {
          if (this.activeGuestEditor) this.activeGuestEditor.destroy()
          if (this.activeGuestSharedEditor) this.activeGuestSharedEditor.dispose()
        } else {
