...
class LoginDialog {
  ...
  render () {
    ...
    return $.div({tabIndex: -1, on: {blur: this.didBlur}},
      $.p(null, 'Step 1: Visit ', $.a({href: 'https://tachyon.atom.io/login'}, 'tachyon.atom.io/login'), ' to generate an authentication token'),
      ...
      $.div(null,
        $.button(
          {
            type: 'button',
            className: 'btn icon inline-block-tight',
        ...
        $.button(
          {
            type: 'button',
            className: 'btn btn-primary icon icon-check inline-block-tight',
            onClick: () => {
